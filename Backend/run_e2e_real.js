...
async function runTests() {
    console.log("====================================================");
    console.log("   STARTING REAL ENVIRONMENT SELENIUM TESTS         ");
    console.log("====================================================");
    ...
    try {
        ...
        try {
            ...
            if (body) {
                ...
                if (tc) {
                    tc["Status"] = "PASS";
                    tc["Remarks / Comments"] = `Selenium verified. Landing page loaded successfully. Title: ${title}`;
                }
            } else {
        ...
        // --- FT_003: User Login - Valid credentials ---
        try {
            ...
            // Enter test account details from users.json
            await emailInput.sendKeys("nikhila@gmail.com");
            ...
            if (currentUrl.includes("/welcome") || currentUrl.includes("/home")) {
                console.log(" -> FT_003 (Login View Render & Submit): PASSED");
                const tc = TEST_CASES.find(t => t["Test ID"] === "FT_003");
                if (tc) {
                    tc["Status"] = "PASS";
                    tc["Remarks / Comments"] = `Selenium verified. Logged in successfully and redirected to: ${currentUrl}`;
                }
            } else {
                throw new Error(`Login redirection failed. Current URL: ${currentUrl}`);
            }
        } catch (err) {
            console.log(` -> FT_003 (Login View Render & Submit): FAILED (${err.message})`);
            const tc = TEST_CASES.find(t => t["Test ID"] === "FT_003");
            if (tc) {
                tc["Status"] = "FAIL";
                tc["Remarks / Comments"] = `Selenium verification failed: ${err.message}`;
        ...
        try {
            // Make sure we access dashboard shell
            ...
            if (currentUrl.includes("/home")) {
                ...
                if (tc) {
                    tc["Status"] = "PASS";
                    tc["Remarks / Comments"] = `Selenium verified. App shell dashboard loaded successfully. URL: ${currentUrl}`;
                }
            } else {
                throw new Error(`Dashboard page route load failed. Current URL: ${currentUrl}`);
            }
        } catch (err) {
        ...
        try {
            ...
            await new Promise(resolve => setTimeout(resolve, 2000));
            const currentUrl = await driver.getCurrentUrl();
            if (currentUrl.includes("/learn/quiz")) {
                console.log(" -> FT_018 (Quiz Form Render): PASSED");
                const tc = TEST_CASES.find(t => t["Test ID"] === "FT_018");
                if (tc) {
                    tc["Status"] = "PASS";
                    tc["Remarks / Comments"] = `Selenium verified. Quiz generator form inputs are active. URL: ${currentUrl}`;
                }
            } else {
                throw new Error(`Quiz form page route load failed. Current URL: ${currentUrl}`);
            }
        } catch (err) {
...
`;
    ...
    // Check if any functional test failed and exit with 1 if true
    const failedTests = TEST_CASES.filter(tc => tc["Status"] === "FAIL");
    if (failedTests.length > 0) {
        console.log(`\n❌ E2E QA Test Run failed. ${failedTests.length} tests failed during execution.`);
        process.exit(1);
    } else {
        console.log("\n🚀 All E2E QA tests passed successfully!");
        process.exit(0);
    }
}
